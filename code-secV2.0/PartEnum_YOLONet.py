# Stage-wise partition by full enumeration (YOLONet, kernel/stride/pad)
...
class StagePartition:
    """DP-based stage partition for YOLONet with kernel/stride/padding."""
    ...
    def load_config(self):
        ...
        # with open('data\\YOLONet.csv','r') as csvfile:
        ...
        for j in range(L):
            ...
            if curr_num == 1:  # 第一阶段只有一个设备
                ...
                if self.layer_type[j] == 3 or self.layer_type[j] == 2:  # 卷积层和池化层，对于YOLONet
                    self.row_start[curr_stage, j] = self.row_start[curr_stage, j] * self.stride_sz[j] - self.pad_sz[j]  # 对于YOLONet
                ...
                t_stage = max(tcomp, tcomm)  # 当前阶段时间
                t_infer = tcomp + tcomm  # 单个输入阶段推理时间
                # print("通信量",tcomm,"计算量",tcomp)   # 计算量应该逐渐增大，arr_device[n]是否计算有误
