# Stage-wise partition by full enumeration of device order (AlexNet)
...
class StagePartition:
    """DP-based stage partition with per-layer receptive field handling."""

    def __init__(self):
        self.bw_matrix = np.zeros((N, N))
        self.flops_per_layer = np.zeros(0)
        self.data_per_layer = np.zeros(0)
        self.layer_height = np.zeros(0)
        self.layer_type = np.zeros(0)
        self.device_perf = np.zeros(0)
        self.dev_stage_map = np.zeros((1, N))
        self.num_stages = 1
        self.row_start = np.zeros((N, L))
        self.row_end = np.zeros((N, L))
        self.stage_time_mat = np.zeros(1)
        self.split_idx_mat = np.zeros(1)
        self.infer_time_mat = np.zeros(1)
        self.kernel_sz = np.zeros(0)
        self.stride_sz = np.zeros(0)
        self.pad_sz = np.zeros(0)

    def load_config(self):
        bw_vals = np.random.randint(21, 31, size=int(N * (N - 1) / 2))
        self.device_perf = np.linspace(81, 100, num=N)
        print("Device perf (GFlops/s):", self.device_perf.shape)
        off = 0
        for i in range(N - 1):
        ...
        print("Link bandwidth (MB/s):", self.bw_matrix.shape)
        ...
        # with open('data\\AlexNet.csv','r') as csvfile:
    ...
    def align_stage_order(self):
        self.num_stages = 1
        order_idx = np.argsort(posInt)
        if not isinstance(order_idx, np.ndarray):
            order_idx = np.array(order_idx)
        curr_val = posInt[0, order_idx[0, 0]]
        self.dev_stage_map[0, order_idx[0, 0]] = self.num_stages
        for i in range(1, order_idx.size):
            if posInt[0, order_idx[0, i]] == curr_val:
                self.dev_stage_map[0, order_idx[0, i]] = self.num_stages
            else:
                curr_val = posInt[0, order_idx[0, i]]
                self.num_stages += 1
                self.dev_stage_map[0, order_idx[0, i]] = self.num_stages

    def run_dp(self):
        # First stage: max period over first j layers with inner-layer split
        ...
        for j in range(L):
            ...
            if curr_num == 1:  # 第一阶段只有一个设备
                ...
                arr_fn = [self.device_perf[curr_stage[i]] for i in range(curr_num)]  # 当前阶段对应的设备性能
                sum_fn = sum(arr_fn)
                index = 0  # 起始行数为第一行
                for i in range(curr_num):
                    self.row_start[curr_stage[i], j] = index  # 分配给设备curr_stage[i]的起始行数
                    index = index + np.round(arr_fn[i] / sum_fn * self.layer_height[j])  #4舍6入5凑偶
                ...
                # 激活层和批归一化层输入输出一致
                if self.layer_type[j] == 3 or self.layer_type[j] == 2:  # 卷积层和池化层，对于AlexNet
                    self.row_start[curr_stage, j] = self.row_start[curr_stage, j] * self.stride_sz[j] - self.pad_sz[j]  # 对于AlexNet
                    self.row_end[curr_stage, j] = (self.row_end[curr_stage, j] - 1) * self.stride_sz[j] + self.kernel_sz[j] - self.pad_sz[j]
                #print("原arr_device=",arr_device)
                arr_device += (self.row_end[curr_stage, j] - self.row_start[curr_stage, j]) / self.layer_height[j] * self.flops_per_layer[j]
                #print("每一个设备分配的计算量",(self.row_end[curr_stage, j] - self.row_start[curr_stage, j]) / self.layer_height[j] * self.flops_per_layer[j])
                #print("更新后arr_device=",arr_device)
                ...
                for m in range(1, j + 1):  # j-m=0->m=j+1
                    #print("第",j-m+1,"层")
                    # print("原每个设备起始层:",self.row_start[curr_stage, j - m + 1])
                    # print("原每个设备终止层:", self.row_end[curr_stage, j - m + 1])
                    if self.layer_type[j - m] == 3 or self.layer_type[j-m] == 2:  # 卷积层和池化层
                        # print("池化层或卷积层")
                        self.row_start[curr_stage, j - m] = self.row_start[curr_stage, j - m + 1] * self.stride_sz[j - m] - self.pad_sz[j - m]
                        self.row_end[curr_stage, j - m] = (self.row_end[curr_stage, j - m + 1] - 1) * self.stride_sz[j - m] +\
                            self.kernel_sz[j - m] - self.pad_sz[j - m]
                    else: # 激活层输入输出不变
                        #print("激活层")
                        self.row_start[curr_stage, j - m] = self.row_start[curr_stage, j - m + 1]
                        self.row_end[curr_stage, j - m] = self.row_end[curr_stage, j - m + 1]
                    # print("回溯后每个设备起始层:", self.row_start[curr_stage, j - m])
                    # print("回溯后每个设备终止层:", self.row_end[curr_stage, j - m])
                    #print("原arr_device=", arr_device)
                    add_cl = (self.row_end[curr_stage, j - m] - self.row_start[curr_stage, j - m]) / self.layer_height[j - m] * self.flops_per_layer[j - m]
                    arr_device += add_cl
                    # print("每一个设备当前层分配的行数", self.row_end[curr_stage, j - m] - self.row_start[curr_stage, j - m])
                    # print("每一个设备当前层分配的计算量",add_cl)
                    # print("向前回溯一层后arr_device=", arr_device)
                    ...
                    # print("去除填充后每个设备起始层:", self.row_start[curr_stage, j - m])
                    # print("去除填充后每个设备终止层:", self.row_end[curr_stage, j - m])
                ...
                for n in range(curr_num):
                    ...
                    # print("第", n, "个设备计算量",arr_device[n])
                    # print("第",n,"个设备计算时间",temp)
                    if temp > tcomp:
                        tcomp = temp
                t_stage = max(tcomp, tcomm)  # 当前阶段时间
                t_infer = tcomp + tcomm  # 单个输入阶段推理时间
                # print("通信时间", tcomm, "计算时间", tcomp)
                # print("当前阶段时间：",t_stage)
                # print("当前阶段推理时间：", t_infer)
                # 每个设备计算时间几乎相同，因为按照计算性能进行行数拆分
                self.stage_time_mat[j, 0] = t_stage
                self.split_idx_mat[j, 0] = 0  # 初始化为0而不是1,索引从0开始的
                self.infer_time_mat[j, 0] = t_infer
        # print("第一阶段执行所有层吞吐量:",1/self.stage_time_mat[L-1, 0])
        # print("第一阶段执行所有推理时间:", self.infer_time_mat[L - 1, 0])
                    ...
                    if curr_num == 1:  # 当前阶段只有一个设备，则前一阶段可能为多个设备
                        ...
                        if k < j:  # 当前阶段参与计算
                            ...
                            if self.layer_type[j] == 3 or self.layer_type[j] == 2:  # 卷积层和池化层
                                self.row_start[curr_stage, j] = self.row_start[curr_stage, j] * self.stride_sz[j] - self.pad_sz[j]  # 对于AlexNet
                                self.row_end[curr_stage, j] = (self.row_end[curr_stage, j] - 1) * self.stride_sz[j] + \
                                    self.kernel_sz[j] - self.pad_sz[j]
                            arr_device += (self.row_end[curr_stage, j] - self.row_start[curr_stage, j]) / self.layer_height[j] * self.flops_per_layer[j]
                            # 填充数据不需要传输，Hs，He中小于1的赋值1，大于hlayer的赋值为hlayer
                            self.row_start[curr_stage, j]=np.clip(self.row_start[curr_stage, j], a_min=0, a_max=10000)
                            self.row_end[curr_stage, j]=np.clip(self.row_end[curr_stage, j], a_min=-10000, a_max=self.layer_height[j])
                            ...
                            for m in range(1, j - k):  # j-m=k+1->m=j-k-1
                                if self.layer_type[j - m] == 3 or self.layer_type[j - m] == 2:  # 卷积层和池化层
                                    self.row_start[curr_stage, j - m] = self.row_start[curr_stage, j - m + 1] * self.stride_sz[j - m] - \
                                        self.pad_sz[j - m]  # 对于AlexNet
                                    self.row_end[curr_stage, j - m] = (self.row_end[curr_stage, j - m + 1] - 1) * self.stride_sz[j - m] + \
                                        self.kernel_sz[j - m] - self.pad_sz[j - m]
                                else:  # 激活层输入输出不变
                                    self.row_start[curr_stage, j - m] = self.row_start[curr_stage, j - m + 1]
...
if __name__ == '__main__':
    ...
    print("Best order:", pipe1, "Throughput:", th1, "Latency: {:.4f}s".format(ti1))
    print("Worst order:", pipe2, "Throughput:", th2, "Latency: {:.4f}s".format(ti2))
    print("Best/Worst throughput: {:.2%}, latency: {:.2%}".format(th1 / th2, ti1 / ti2))
