# PSO + DP stage partition (AlexNet, kernel/stride/padding)
...
class PSOAgent:
    ...
    def run_dp(self):
        ...
        for j in range(L):   # 第一阶段设备肯定不是中继节点
            if curr_num == 1:  # 第一阶段只有一个设备
                ...
                if self.layer_type[j] == 3 or self.layer_type[j] == 2:  # 卷积层和池化层，对于AlexNet
                    self.row_start[curr_stage, j] = self.row_start[curr_stage, j]*solver.stride_sz[j]-solver.pad_sz[j]    # 对于AlexNet
                    ...
                    if curr_num == 1:  # 当前阶段只有一个设备，则前一阶段可能为多个设备
                        ...
                        if k < j:  # 当前阶段参与计算
                            ...
                            for m in range(1, j - k):  # j-m=k+1->m=j-k-1
                                if self.layer_type[j-m] == 3 or self.layer_type[j-m] == 2:  # 卷积层和池化层
                                    self.row_start[curr_stage, j-m] = self.row_start[curr_stage, j-m+1] * solver.stride_sz[j-m] - solver.pad_sz[j-m]  # 对于AlexNet
                                    self.row_end[curr_stage, j-m] = (self.row_end[curr_stage, j-m+1] - 1) * solver.stride_sz[j-m] + \
                                        solver.kernel_sz[j-m] - solver.pad_sz[j-m]
                                else:  # 激活层输入输出不变
                                    self.row_start[curr_stage, j - m] = self.row_start[curr_stage, j - m + 1]
...
class SwarmDPSolver:
    def __init__(self, max_vel, tol, C1=2, C2=2, W=1):
        ...
        self.best_position = np.zeros((1, N))
        self.best_intPosition = np.zeros((1, N))
    ...
    def update_ndim(self):
        ...
        print("Initial best fitness:", self.get_best_fitness_value())
        ...
        for i in range(1, self.iter_num + 1):
            ...
            print("Best fitness:", self.get_best_fitness_value(), "Latency:", self.inference_time)
            print("Best position:", np.around(self.get_best_position()))
    ...
    def visual(self):
        ...
        plt.title("PSO+DP (agents={}, iters={}), seed={}".format(size, iter_num, seed), fontsize=25, backgroundcolor='#3c7f99',
...
if __name__ == '__main__':
    N = 3            # 粒子的维度=设备数量
    L = 13           # 当前DNN特征提取层的层数(卷积层Conv、池化层Pool、批量归一化层BN、激活层Relu)vgg19模型
    size = 20        # 粒子数量
    # random_size = size*0.6  # 每次迭代随机的粒子数
    iter_num = 20    # 迭代次数
    max_pos = N      # 最大位置范围
