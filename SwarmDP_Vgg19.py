...
class PSOAgent:
    def __init__(self, cl, dl, height, tlayer, band, fn, x_max, max_vel, dim):
        ...
        self.pos = np.random.uniform(1, x_max, (1, dim))       # 粒子的当前位置,管道的阶段数最大为设备数，随机取值
...
if __name__ == '__main__':
    N = 7            # 粒子的维度=设备数量
    L = 37           # 当前DNN特征提取层的层数(卷积层Conv、池化层Pool、批量归一化层BN、激活层Relu)vgg19模型
    size = 20        # 粒子数量
    iter_num = 20    # 迭代次数
    max_pos = N      # 最大位置范围
    seed = 1         # 随机数,固定相同带宽范围取值不变
    start = datetime.now()
    np.random.seed(seed)
